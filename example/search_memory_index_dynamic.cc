...
template<typename T>
int search_memory_index(lambda::Metric &metric, const std::string &index_path,
    ...
    auto rs = lambda::binary_file::load_aligned_bin<T>(query_file, query, query_num, query_dim,
                                query_aligned_dim);
    if(!rs.is_ok()) {
        return -1;
    }
    ...
    if (truthset_file != std::string("null") && melon::exists(truthset_file)) {
        lambda::load_truthset(truthset_file, gt_ids, gt_dists, gt_num, gt_dim);
        if (gt_num != query_num) {
        ...
        std::cout << " Truthset file " << truthset_file
                  << " not found. Not computing recall." << std::endl;
    }

    // Load the index
    // lambda::Index<T, uint32_t> index(metric, query_dim, 0, false);
    lambda::Index<T, uint32_t> index(metric, query_dim, 0, true, true, false);
    index.load(index_path.c_str(), num_threads,
               *(std::max_element(Lvec.begin(), Lvec.end())));
    std::cout << "Index loaded" << std::endl;
    if (metric == lambda::FAST_L2)
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    for (auto L : Lvec) {
        ...
        // lambda::save_bin<uint32_t>(cur_result_path,

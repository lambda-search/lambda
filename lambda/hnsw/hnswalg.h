...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(SpaceInterface<dist_t> *s, size_t max_elements, size_t M = 16,
                        size_t kr_construction = 200, size_t random_seed = 100)
            ...
            maxM_ = M_;
            maxM0_ = M_ * 2;
            ef_construction_ = std::max(kr_construction, M_);
            _kr = 10;
        ...
        struct CompareByFirst {
            constexpr bool operator()(
                    std::pair<dist_t, tableint> const &a, std::pair<dist_t, tableint> const &b) const noexcept {
        ...
        inline labeltype *getExternalLabeLp(tableint internal_id) const {
            return (
                    labeltype *) (data_level0_memory_ + internal_id * size_data_per_element_ + label_offset_);
        }
...
//                    if (candidate_id == 0) continue;
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>,
                CompareByFirst>
        searchBaseLayerST(
                tableint ep_id, const void *data_point, const bitmap_t *bitmap, size_t kr) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            if ((!has_deletions || !isMarkedDeleted(ep_id))
                && (!bitmap || bitmap->contains(uint32_t(getExternalLabel(ep_id))))) {
                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
                lowerBound = dist;
                top_candidates.emplace(dist, ep_id);
            ...
            while (!candidate_set.empty()) {
                ...
                _mm_prefetch((char *) (visited_array + *(data + 1) + 64), _MM_HINT_T0);
                _mm_prefetch(
                        data_level0_memory_ + (*(data + 1)) * size_data_per_element_ + offsetData_, _MM_HINT_T0);
...
//                    if (candidate_id == 0) continue;
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        if (top_candidates.size() < kr || lowerBound > dist) {
                            ...
                            if ((!has_deletions || !isMarkedDeleted(candidate_id))
                                && (!bitmap || bitmap->contains(uint32_t(getExternalLabel(candidate_id)))))
                                top_candidates.emplace(dist, candidate_id);

                            if (top_candidates.size() > kr)
                                top_candidates.pop();
        ...
        std::priority_queue<std::pair<dist_t, tableint>> searchKnnInternal(void *query_data, int k) {
            ...
            if (has_deletions_) {
                std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
                        searchBaseLayerST<true>(currObj, query_data, _kr);
                top_candidates.swap(top_candidates1);
            } else {
                std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
                        searchBaseLayerST<false>(currObj, query_data, _kr);
                top_candidates.swap(top_candidates1);
            }
        ...
        void loadIndex(
                const std::string &location, SpaceInterface<dist_t> *s, size_t max_elements_i = 0) {
        ...
        std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(
                const void *query_data, const bitmap_t *bitmap, size_t k) const {
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>,
                    ...
                    top_candidates;
            if (has_deletions_) {
                top_candidates = searchBaseLayerST<true, true>(currObj, query_data, bitmap, std::max(_kr, k));
            } else {
                top_candidates =
                        searchBaseLayerST<false, true>(currObj, query_data, bitmap, std::max(_kr, k));
            }

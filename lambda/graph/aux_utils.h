...
namespace lambda {
    ...
    MELON_EXPORT double get_memory_budget(const std::string &mem_budget_str);

    MELON_EXPORT double get_memory_budget(double search_ram_budget_in_gb);

    MELON_EXPORT melon::result_status add_new_file_to_single_index(std::string index_file,
                                                                   std::string new_file);

    MELON_EXPORT size_t calculate_num_pq_chunks(double final_index_ram_limit,
                                                size_t points_num,
                                                uint32_t dim);

    MELON_EXPORT double calculate_recall(
    ...
    MELON_EXPORT double calculate_range_search_recall(
            unsigned num_queries, std::vector<std::vector<uint32_t>> &groundtruth,
            std::vector<std::vector<uint32_t>> &our_results);

    [[nodiscard]] MELON_EXPORT melon::result_status read_idmap(const std::string &fname,
                                 std::vector<unsigned> &ivecs);

    template<typename T>
    [[nodiscard]] MELON_EXPORT std::pair<melon::result_status,T*> load_warmup(const std::string &cache_warmup_file,
                                uint64_t &warmup_num, uint64_t warmup_dim,
                                uint64_t warmup_aligned_dim);

    [[nodiscard]] MELON_EXPORT melon::result_status merge_shards(const std::string &vamana_prefix,
    ...
    template<typename T>
    MELON_EXPORT std::string preprocess_base_file(
            const std::string &infile, const std::string &indexPrefix,
            lambda::Metric &distMetric);

    template<typename T>
    [[nodiscard]] MELON_EXPORT melon::result_status build_merged_vamana_index(
    ...
    template<typename T>
    MELON_EXPORT uint32_t optimize_beamwidth(
    ...
    template<typename T>
    [[nodiscard]] MELON_EXPORT melon::result_status build_disk_index(const char *dataFilePath,
    ...
    template<typename T>
    [[nodiscard]] MELON_EXPORT melon::result_status create_disk_layout(

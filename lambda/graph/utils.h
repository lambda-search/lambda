...
#define BUFFER_SIZE_FOR_CACHED_IO (uint64_t) 1024 * (uint64_t) 1048576
    ...
    inline bool file_exists(const std::string &name, bool dirCheck = false) {
        ...
        if (val != 0) {
            switch (errno) {
                case EINVAL:
                    FLARE_LOG(INFO) << "Invalid argument passed to stat()";
                    break;
                case ENOENT:
                    // file is not existing, not an issue, so we won't cout anything.
                    break;
                default:
                    FLARE_LOG(INFO) << "Unexpected error in stat():" << errno;
                    break;
            }
            return false;
        } else {
    ...
    inline flare::result_status load_range_truthset(const std::string &bin_file,
        ...
        FLARE_LOG(INFO) << "Metadata: #pts = " << gt_num
...
#pragma omp parallel for schedule(static, 65536)
    ...
    float prepare_base_for_inner_products(const std::string in_file,
        ...
        FLARE_LOG(DEBUG) << "Pre-processing base file by adding extra coordinate";
    ...
    template<typename T>
    inline flare::result_status copy_aligned_data_from_file(const char *bin_file, T *&data,
                                                            size_t &npts, size_t &dim,
                                                            const size_t &rounded_dim,
                                                            size_t offset = 0) {
        if (data == nullptr) {
            FLARE_LOG(ERROR) << "Memory was not allocated for " << data
                             << " before calling the load function. Exiting...";
            return flare::result_status(-1, "Null pointer passed to copy_aligned_data_from_file function");
        }
        std::ifstream reader;
    ...
    FLARE_EXPORT void normalize_data_file(const std::string &inFileName,
    ...
    inline flare::result_status validate_index_file_size(std::ifstream &in) {
        if (!in.is_open())
        ...
        in.seekg(0, in.beg);
        if (actual_file_size != expected_file_size) {
            FLARE_LOG(ERROR) << "Index file size error. Expected size (metadata): "
                             << expected_file_size
                             << ", actual file size : " << actual_file_size << ".";
            return flare::result_status(-1, "bad file size");
        }
        return flare::result_status::success();
    }

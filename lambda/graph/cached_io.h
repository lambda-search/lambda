...
// sequential cached reads
    ...
    public:
        ...
        flare::result_status open(const std::string &filename, uint64_t cacheSize) {
            this->cur_off = 0;
            ...
            try {
                ...
                FLARE_LOG(INFO) << "Opened: " << filename.c_str() << ", size: " << fsize
                                << ", cache_size: " << cacheSize;
            } catch (std::system_error &e) {
                return flare::result_status(-1, filename);
            }
        ...
        [[nodiscard]] flare::result_status read(char *read_buf, uint64_t n_bytes) {
    ...
    public:
        cached_ofstream(const std::string &filename, uint64_t cache_size)
            ...
            try {
                ...
                FLARE_LOG(INFO) << "Opened: " << filename.c_str()
        ...
        void close() {
            ...
            if (writer.is_open())
                writer.close();
            FLARE_LOG(INFO) << "Finished writing " << fsize << "B";
        }

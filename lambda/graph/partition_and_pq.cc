...
#pragma omp parallel for schedule(static, 65536)
        ...
        size_t tmp_size;
        auto rs = lambda::binary_file::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                                       (size_t) num_centers, dim, cumul_bytes[0], &tmp_size);
        if (!rs.is_ok()) {
            return rs;
        }
        cumul_bytes[1] = cumul_bytes[0] + tmp_size;
        rs = lambda::binary_file::save_bin<float>(
                pq_pivots_path.c_str(), centroid.get(),
                (size_t) dim, 1, cumul_bytes[1], &tmp_size);
        if (!rs.is_ok()) {
            return rs;
        }
        cumul_bytes[2] = cumul_bytes[1] + tmp_size;
        rs = lambda::binary_file::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                                     chunk_offsets.size(), 1, cumul_bytes[2], &tmp_size);
        if (!rs.is_ok()) {
            return rs;
        }
        cumul_bytes[3] = cumul_bytes[2] + tmp_size;

        rs = lambda::binary_file::save_bin<uint64_t>(pq_pivots_path.c_str(), (uint64_t *) cumul_bytes.data(),
...
#pragma omp parallel for schedule(static, 65536)
            ...
            // rotation matrix
            uint32_t errcode = LAPACKE_sgesdd(
        ...
        size_t tmp_size;
        cumul_bytes[0] = METADATA_SIZE;
        auto rs = lambda::binary_file::save_bin<float>(opq_pivots_path.c_str(), full_pivot_data.get(),
                                                       (size_t) num_centers, dim, cumul_bytes[0], &tmp_size);
        if (!rs.is_ok()) {
            return rs;
        }
        ...
        cumul_bytes[2] = cumul_bytes[1] + tmp_size;
        rs = lambda::binary_file::save_bin<uint32_t>(opq_pivots_path.c_str(), chunk_offsets.data(),
        ...
        rs = lambda::binary_file::save_bin<uint64_t>(opq_pivots_path.c_str(), (uint64_t *) cumul_bytes.data(),
                                                     cumul_bytes.size(), (size_t) 1, (size_t) 0);
        if (!rs.is_ok()) {
            return rs;
        }
        ...
        rs = lambda::binary_file::save_bin<float>(rotmat_path.c_str(), rotmat_tr.get(), dim, dim);
        return rs;
    }
    ...
    flare::result_status generate_pq_data_from_pivots(const std::string data_file,
        ...
        if (!file_exists(pq_pivots_path)) {
            ...
            auto rs = lambda::binary_file::load_bin<uint64_t>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);
            if (!rs.is_ok()) {
                return rs;
            }
            if (nr != 4) {
            ...
            rs = lambda::binary_file::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc,
                                                      file_offset_data[0]);
            if (!rs.is_ok()) {
                return rs;
            }
            ...
            rs = lambda::binary_file::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc,
                                                      file_offset_data[1]);
            if (!rs.is_ok()) {
                return rs;
            }
            ...
            rs = lambda::binary_file::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc,
                                                         file_offset_data[2]);
            if (!rs.is_ok()) {
                return rs;
            }
            ...
            if (use_opq) {
                ...
                rs = lambda::binary_file::load_bin<float>(rotmat_path.c_str(), rotmat_tr, nr, nc);
                if (!rs.is_ok()) {
                    return rs;
                }
        ...
        std::memset(block_compressed_base.get(), 0,
                    block_size * (uint64_t) num_pq_chunks * sizeof(uint32_t));
...
#pragma omp parallel for schedule(static, 8192)
            ...
            if (num_centers > 256) {
                ...
                lambda::convert_types<uint32_t, uint8_t>(
...
// This code should only be available in the "build" DLL.
    ...
    int shard_data_into_clusters(const std::string data_file, float *pivots,
        ...
        for (size_t block = 0; block < num_blocks; block++) {
            ...
            base_reader.read((char *) block_data_T.get(),
                             sizeof(T) * (cur_blk_size * dim));
            lambda::convert_types<T, float>(block_data_T.get(), block_data_float.get(),
                                            cur_blk_size, dim);
...
// each shard, and retrieve the actual vectors on demand.
    ...
    flare::result_status retrieve_shard_data_from_ids(const std::string data_file,
        ...
        auto rs = lambda::binary_file::load_bin<uint32_t>(idmap_filename, shard_ids, shard_size, tmp);
        if (!rs.is_ok()) {
            return rs;
        }
        uint32_t cur_pos = 0;
    ...
    flare::result_status partition_with_ram_budget(const std::string data_file,
        ...
        while (!fit_in_ram) {
            ...
            lambda::kmeans::run_lloyds(train_data_float, num_train, train_dim, pivot_data,
                                       num_parts, max_k_means_reps, NULL, NULL);
            ...
            for (auto &p : cluster_sizes) {
                ...
                double cur_shard_ram_estimate =
                        lambda::estimate_ram_usage(p, train_dim, sizeof(T), graph_degree);

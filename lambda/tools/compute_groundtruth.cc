...
void exact_knn(
        ...
        lambda::Metric metric = lambda::Metric::L2)  // queries in Col major
{
    ...
    if (metric == lambda::Metric::COSINE) {  // we convert cosine distance as
...
#pragma omp parallel for schedule(static, 4096)
    ...
    if (metric == lambda::Metric::INNER_PRODUCT)
        std::cout << " MIPS ";
    else if (metric == lambda::Metric::COSINE)
        std::cout << " Cosine ";
    else
        std::cout << " L2 ";
    ...
    for (uint64_t b = 0; b < div_round_up(nqueries, q_batch_size); ++b) {
        ...
        if (metric == lambda::Metric::L2 || metric == lambda::Metric::COSINE) {
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    if (metric == lambda::Metric::COSINE) {
        delete[] points;
        delete[] queries;
    }
}
...
int aux_main(const std::string &base_file, const std::string &query_file,
    ...
    if (!tags_file.empty()) {
        ...
        lambda::binary_file::load_bin<std::uint32_t>(tags_file, tag_data, file_num_points,
                                         file_dim);
        if (file_dim != 1) {
        ...
        lambda::get_bin_metadata(base_file, raw_pts, raw_dim);
    ...
    save_groundtruth_as_one_file(gt_file, closest_points, dist_closest_points,
                                 nqueries, k);
    lambda::aligned_free(query_data);
